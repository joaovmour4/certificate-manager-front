...
interface AxiosResponseEmpresa extends AxiosResponse{
  data: Empresa
}
...
const EditCompanyModal = (props: props) => {
  ...
  const handleSubmit = ()=>{
    ...
    api
      .put(`/empresa/${props.idEmpresa}`, data)
      .then(response => {
  ...
  React.useEffect(()=>{
    api
      .get(`/empresa/${props.idEmpresa}`)
      .then((response: AxiosResponseEmpresa) => {
        console.log(response)
        setNameInput(response.data.nameEmpresa)
          ...
          <div className="absolute w-1/3 my-6 mx-auto max-w-3xl">
            <div className="border rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
              <div className="flex items-start justify-between p-5 border-b border-solid border-gray-300 rounded-t ">
                <h3 className="text-3xl font=semibold">Editar Empresa</h3>
              </div>
