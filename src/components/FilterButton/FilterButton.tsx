import React from 'react'
import FilterModal from '../FilterModal/FilterModal'

const FilterButton = () => {
    const buttonRef = React.useRef<HTMLButtonElement>(null)
    const [showFilterModal, setShowFilterModal] = React.useState<boolean>(false)

    const handleFilterModal = () => {
        setShowFilterModal(prevState => !prevState)
    }

  return (
    <div className='flex relative'>
        <button ref={buttonRef} onClick={handleFilterModal} className={`flex gap-x-1 items-center px-3 ${showFilterModal ?'bg-blue' : 'bg-gray-50'} border border-gray-300 text-gray-900 text-sm rounded font-thin`}>
            <img className='h-6' src="data:image/png;base64," alt="filter--v1"></img>
            Filtros
        </button>
        {showFilterModal &&
            <FilterModal
                setShowFilterModal={setShowFilterModal}
                buttonRef={buttonRef}
            />
        }
    </div>
  )
}

export default FilterButton